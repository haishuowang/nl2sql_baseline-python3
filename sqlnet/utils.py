...
def load_dataset(toy=False, use_small=False, mode='train'):
    ...
    root_path = os.getcwd()
    dev_sql, dev_table = load_data(f'{root_path}/data/val/val.json', f'{root_path}/data/val/val.tables.json',
                                   use_small=use_small)
    dev_db = f'{root_path}/data/val/val.db'
    if mode == 'train':
        train_sql, train_table = load_data(f'{root_path}/data/train/train.json',
                                           f'{root_path}/data/train/train.tables.json',
                                           use_small=use_small)
        train_db = f'{root_path}/data/train/train.db'
        return train_sql, train_table, train_db, dev_sql, dev_table, dev_db
    elif mode == 'test':
        test_sql, test_table = load_data(f'{root_path}/data/test/test.json',
                                         f'{root_path}/data/test/test.tables.json',
                                         use_small=use_small)

        test_db = f'{root_path}/data/test/test.db'
